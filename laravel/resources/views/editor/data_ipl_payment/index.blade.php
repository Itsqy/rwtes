...
@section('title', 'Data Pembayaran IPL')   
...
<script>
  var table;
    $(document).ready(function() {
        ...
        ajax: "{{ URL::route('editor.data-ipl-payment.data') }}",
        columns: [
        ...
        { data: 'total_tarif_ipl', name: 'total_tarif_ipl', render: $.fn.dataTable.render.number( ',', '.', 0, 'Rp. ' ) }, 
        ]
      });
    });
    ...
    { 
      ...
      $.ajax({
        ...
        success: function(data)
        {
          ...
          $('.modal-title').text('Edit Data Pembayaran IPL'); // Set title to Bootstrap modal title
        },
    ...
    function update(id)
    {
      ...
      $.ajax({
        ...
        success: function(data) {  
          $('.errorData Pembayaran IPLName').addClass('hidden');
          ...
          if ((data.errors)) {
            ...
            if (data.errors.house_type_name) {
              $('.errorData Pembayaran IPLName').removeClass('hidden');
              $('.errorData Pembayaran IPLName').text(data.errors.house_type_name);
            }
          } else {
          var options = { 
            "positionClass": "toast-bottom-right", 
